...
export function discoverColumns(items: TelemetryItem[]): ColumnDef[] {
  const seen = new Set<string>();
  const cols: ColumnDef[] = [];

  function add(key: string, label: string, category: ColumnDef['category']) {
    if (seen.has(key)) return;
    seen.add(key);
    cols.push({ key, label, category });
  }

  const BD_FIELDS = ['duration', 'responseCode', 'success', 'url', 'name', 'resultCode', 'type', 'target', 'data', 'message', 'severityLevel'];
  ...
  for (const item of items) {
    ...
    if (bd) {
      for (const f of BD_FIELDS) {
        if (f in bd && (bd as unknown as Record<string, unknown>)[f] !== undefined) {
          add(`baseData.${f}`, f, 'baseData');
        }
      }
    }

    if (item.envelope.tags) {
      for (const key of Object.keys(item.envelope.tags)) {
        add(`tags.${key}`, key.replace('ai.', ''), 'tags');
      }
    }

    if (bd && 'properties' in bd) {
      const props = (bd as { properties?: Record<string, string> }).properties;
      if (props) {
        for (const key of Object.keys(props)) {
          add(`properties.${key}`, key, 'properties');
        }
      }
    }

    if (bd && 'measurements' in bd) {
      const ms = (bd as { measurements?: Record<string, number> }).measurements;
      if (ms) {
        for (const key of Object.keys(ms)) {
          add(`measurements.${key}`, key, 'measurements');
        }
      }
    }
  }
